...
static int read_db(int *monthly_budget, int *spent) {
	...
	if((fp = fopen(DATA_FILE, "r")) == NULL) {
		fprintf(stderr, "ERROR: Cannot open '%s'.\n", DATA_FILE);
		fprintf(stderr, "       Maybe you forgot to init a new month.\n");
		return EXIT_FAILURE;
	}
...
static int print_budget(int summary) {
	struct tm *stime;
...
int main(int argc, char **argv) {
	...
	while((opt = getopt(argc, argv, "hi:s:t")) != -1) {
		switch(opt) {
	...
	return print_budget(print_summary);
}
